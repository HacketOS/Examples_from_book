...
def batch(data, batch_n):
    inds = range(data,shape[0])
    np.random.shuffle(inds)
    for i in range(data[0] / batch_n):
        ii = inds[i*batch_n:(i+1)*batch_n]
        yield data[ii,:]

def he_initializer(size):
    return tf.random_normal_initializer(mean = 0,
...
class AAE(object):
    def __init__(self, batch_size = 64, input_space = 28*28, latent_space =10,
     p = 3., middle_layers = None, activation_fn = tf.nn.tanh, learning_rate = 0.001,
    ...
    def _discriminator(self, tensor, sizes):
        ...
        with tf.variable_scope('clas-layer'):
    ...
    def train(self):
        ...
        for i in range(1000):
            ...
            if gloss > np.log(self.p):
                ...
                gloss, _ = sess.run([self.enc_loss, self.train_discriminator], feed_dict={self.input_x: batch_z})
